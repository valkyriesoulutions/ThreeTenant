...
using ThreeTenant.Identity;
using ThreeTenant.Schemas;
using ThreeTenant.Web.Chat.SignalR;
using ThreeTenant.Web.Common;
using ThreeTenant.Web.Resources;
using Swashbuckle.AspNetCore.Swagger;
using ThreeTenant.Web.Swagger;
using Stripe;
...
using ThreeTenant.Web.HealthCheck;
using Owl.reCAPTCHA;
using Microsoft.AspNetCore.Server.Kestrel.Https;
using Microsoft.Extensions.DependencyInjection.Extensions;
using ThreeTenant.Web.Extensions;
using ThreeTenant.Web.MultiTenancy;
using SecurityStampValidatorCallback = ThreeTenant.Identity.SecurityStampValidatorCallback;
...
namespace ThreeTenant.Web.Startup
{
    public class Startup
    {
        ...
        public IServiceProvider ConfigureServices(IServiceCollection services)
        {
            ...
            return services.AddAbp<ThreeTenantWebMvcModule>(options =>
            {
        ...
        public void Configure(IApplicationBuilder app, IWebHostEnvironment env, ILoggerFactory loggerFactory)
        {
            ...
            if (env.IsDevelopment())
            {
                ...
                app.UseThreeTenantForwardedHeaders();
            }
            else
            {
            ...
            if (ThreeTenantConsts.PreventNotExistingTenantSubdomains)
            {
            ...
            if (WebConsts.SwaggerUiEnabled)
            {
                ...
                {
                    options.SwaggerEndpoint(_appConfiguration["App:SwaggerEndPoint"], "ThreeTenant API V1");
                    options.IndexStream = () => Assembly.GetExecutingAssembly()
                        .GetManifestResourceStream("ThreeTenant.Web.wwwroot.swagger.ui.index.html");
        ...
        private void ConfigureSwagger(IServiceCollection services)
        {
            ...
            {
                options.SwaggerDoc("v1", new OpenApiInfo() {Title = "ThreeTenant API", Version = "v1"});
                ...
                var applicationXml = $"ThreeTenant.Application.xml";
                var applicationXmlPath = Path.Combine(AppContext.BaseDirectory, applicationXml);
                options.IncludeXmlComments(applicationXmlPath);

                var webCoreXmlFile = $"ThreeTenant.Web.Core.xml";
